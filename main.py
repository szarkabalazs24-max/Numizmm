...
def embed(title, desc, color):
    ...
    e.set_footer(text="✨ Numiz’s MM Basement ✨")
    return e
...
async def on_member_join(member):
    ...
    if ch:
        await ch.send(
            f"👋 {member.mention} üdv a szerveren!\n"
...
async def warn_info(i: discord.Interaction, tag: discord.Member):
    ...
    if not warns:
        return await i.response.send_message("ℹ️ Nincs figyelmeztetés", ephemeral=True)

    text = "\n".join([f"**{idx}.** {w}" for idx, w in enumerate(warns, 1)])
